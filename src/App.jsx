        ...
        <div style={{ background: "rgba(255,255,255,0.03)", borderRadius: 16, padding: 24, border: `2px solid ${li.border}`, position: "relative", overflow: "hidden" }}>
          ...
          {selected.id === "crowd" && (() => {
            ...
            return (<>
              ...
              {/* 시간별 체류/누적 추이 그래프 */}
              {(settings.hourlyLog || []).length >= 2 && <div style={{ marginBottom: 16 }}>
                <h3 style={{ color: "#8892b0", fontSize: 13, marginBottom: 8 }}>📈 체류 / 누적 추이</h3>
                <ResponsiveContainer width="100%" height={200}>
                  <LineChart data={(settings.hourlyLog || []).slice(-60).map(h => ({ time: h.time, 체류: h.current || 0, 누적: h.cumulative || 0 }))} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
...
function CounterPage({ categories, setCategories, settings, setSettings, session }) {
  ...
  // 초기 로드: crowdDB가 유일한 진실 → categories에 반영
  useEffect(() => {
    if (window.crowdDB) {
      window.crowdDB.get().then(data => {
        if (data) {
          const t = data.total || 0;
          const c = data.cumulative || 0;
          setCurTotal(t); curRef.current = t;
          setCumTotal(c); cumRef.current = c;
          setCategories(p => p.map(cat => cat.id === "crowd" ? { ...cat, currentValue: t } : cat));
...
function CMSPage({ categories, setCategories, settings, setSettings, alerts, setAlerts, smsLog, initialTab, initialCatId, extraTabs, onExtraTab, userRole, accounts, setAccounts, onDataReset }) {
  ...
  const baseTabs = [
    ...
    { id: "sms", label: "📱 SMS" }, { id: "zones", label: "🗺️ 구역" }, { id: "workers", label: "👷 근무자" }, { id: "parking", label: "🅿️ 주차장" }, { id: "shuttlecms", label: "🚌 셔틀" }, { id: "crowdcms", label: "👥 인파관리" }, { id: "custom", label: "➕ 항목" },
