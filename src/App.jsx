...
function CounterPage({ categories, setCategories, settings, setSettings, session }) {
    ...
    if (window.crowdDB) {
      window.crowdDB.get().then(data => {
        if (data) {
          ...
          setCumTotal(data.cumulative || 0);
          if (data.zones?.length) setZoneData(prev => prev.map(z => {
            const d = data.zones.find(dz => dz.id === z.id);
            return d ? { ...z, count: d.count || 0, cumulative: d.cumulative || 0 } : z;
          }));
        }
      });
    }
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.detail) {
        setCurTotal(e.detail.total || 0);
        setCumTotal(e.detail.cumulative || 0);
        if (e.detail.zones?.length) setZoneData(prev => prev.map(z => {
          const d = e.detail.zones.find(dz => dz.id === z.id);
          return d ? { ...z, count: d.count || 0, cumulative: d.cumulative || 0 } : z;
        }));
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      ...
      const entry = { time: fmtHM(new Date()), date: new Date().toLocaleDateString("ko-KR"), current: curTotal, cumulative: cumTotal, zones: zoneData.filter(z => z.name).map(z => ({ name: z.name, current: z.count || 0, cumulative: z.cumulative || 0 })) };
      setSettings(prev => ({ ...prev, hourlyLog: [...(prev.hourlyLog || []).slice(-288), entry] }));
    }, 60000);
    return () => clearInterval(iv);
  }, [curTotal, cumTotal]);

  // ★ 핵심: 빠른 카운터 — setSettings 호출 안 함
  const adjustTotal = (d) => {
    const ts = fmtTime(new Date());
    const zoneName = selZone ? (zoneData.find(z => z.id === selZone)?.name || "") : "";
    ...
    let newZones = zoneData;
    if (selZone) {
      newZones = zoneData.map(z => z.id === selZone ? { ...z, count: Math.max(0, (z.count || 0) + d), cumulative: d > 0 ? (z.cumulative || 0) + d : (z.cumulative || 0) } : z);
