    ...
    if (this.state.error) {
      return (<div style={{ minHeight: "100vh", background: "#0a0a1a", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "sans-serif" }}>
        <div style={{ maxWidth: 420, textAlign: "center" }}>
          <div style={{ fontSize: 64, marginBottom: 16 }}>💥</div>
          <h2 style={{ color: "#F44336", fontSize: 22, fontWeight: 800, margin: "0 0 12px" }}>앱 오류</h2>
          <p style={{ color: "#8892b0", fontSize: 14, margin: "0 0 8px" }}>{String(this.state.error?.message || this.state.error)}</p>
          <button onClick={() => { localStorage.clear(); sessionStorage.clear(); location.href = location.pathname; }} style={{ width: "100%", padding: 16, borderRadius: 12, border: "none", background: "#F44336", color: "#fff", fontSize: 16, fontWeight: 700, cursor: "pointer", marginTop: 16 }}>🔄 데이터 초기화 후 재시작</button>
          <button onClick={() => this.setState({ error: null })} style={{ width: "100%", padding: 14, borderRadius: 12, border: "1px solid #333", background: "transparent", color: "#888", fontSize: 14, cursor: "pointer", marginTop: 8 }}>다시 시도</button>
        </div>
      </div>);
    }
...
export default function App() {
  ...
  try {
    return <ErrorBoundary><AppMain onError={setFatalError} /></ErrorBoundary>;
  } catch (e) {
    return (<div style={{ minHeight: "100vh", background: "#0a0a1a", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "sans-serif" }}>
      <div style={{ maxWidth: 400, textAlign: "center" }}>
...
function AppMain({ onError }) {
  ...
  const [outdoor, setOutdoor] = useState(() => localStorage.getItem("_outdoor") === "1");
...
function AuthenticatedApp({ session, accounts, setAccounts, onLogout, initialPage, setPage: setPageExt }) {
  ...
  return (<div className={`desktop-scale${outdoor ? " outdoor" : ""}`} style={{ fontFamily: "'Noto Sans KR',-apple-system,sans-serif" }}>
    <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;400;500;700;800;900&display=swap" rel="stylesheet" />
    <style>{`@keyframes slideIn{from{transform:translateX(120%);opacity:0}to{transform:translateX(0);opacity:1}}@keyframes blink{0%,100%{opacity:1}50%{opacity:.3}}
@media(min-width:768px){body{font-size:16px}.desktop-scale{zoom:1.1}}
@media(min-width:1200px){.desktop-scale{zoom:1.15}}
.outdoor{filter:invert(0.92) hue-rotate(180deg);background:#fff !important}
.outdoor img,.outdoor svg,.outdoor video,.outdoor canvas,.outdoor [style*="emoji"],.outdoor .recharts-surface{filter:invert(1) hue-rotate(180deg)}
.outdoor a[href^="tel"]{filter:invert(1) hue-rotate(180deg);background:rgba(76,175,80,0.25) !important;border-color:rgba(76,175,80,0.5) !important}
.outdoor nav{border-color:#ccc !important}
`}</style>
    ...
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1001, background: "rgba(10,10,26,0.95)", borderBottom: "1px solid #1a1a2e", padding: "6px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", backdropFilter: "blur(10px)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        <button onClick={() => { const next = !outdoor; setOutdoor(next); localStorage.setItem("_outdoor", next ? "1" : "0"); }} style={{ padding: "4px 10px", borderRadius: 6, border: outdoor ? "1px solid #FF9800" : "1px solid #333", background: outdoor ? "rgba(255,152,0,0.15)" : "transparent", color: outdoor ? "#FF9800" : "#556", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>{outdoor ? "☀️" : "🌙"}</button>
