...
function usePersist(key, init) {
  ...
  const set = useCallback((v) => {
    ...
    saveTimer.current = setTimeout(() => {
      // ★ categories 저장 시 crowd.currentValue 제거 (crowdDB가 관리)
      let saveJson = json;
      if (key.includes("_cat_") && Array.isArray(next)) {
        const cleaned = next.map(c => c.id === "crowd" ? { ...c, currentValue: 0 } : c);
        saveJson = JSON.stringify(cleaned);
      }
