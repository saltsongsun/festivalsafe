...
function CounterPage({ categories, setCategories, settings, setSettings, session }) {
  ...
  const [crowdState, setCrowdState] = useState(() => {
    // localStorage에서 빠르게 초기화 (Supabase 로드 전까지)
    try { const d = JSON.parse(localStorage.getItem("_crowd") || "{}"); return { total: d.total || 0, cumulative: d.cumulative || 0, zones: d.zones || [] }; }
    catch { return { total: 0, cumulative: 0, zones: [] }; }
  });
...
function OrgChartTab({ settings, setSettings }) {
  ...
  const [form, setForm] = useState({ name: "", org: "", position: "", phone: "" });
  const [collapsed, setCollapsed] = useState({});
  const [dragId, setDragId] = useState(null);

  const getChildren = (pid) => org.filter(n => n.parentId === pid).sort((a, b) => (a.order || 0) - (b.order || 0));
  const roots = org.filter(n => !n.parentId).sort((a, b) => (a.order || 0) - (b.order || 0));

  const addNode = (parentId) => {
    if (!form.name) { alert("이름을 입력하세요."); return; }
    const node = { id: "org_" + Date.now(), ...form, parentId: parentId || null, order: org.filter(n => n.parentId === (parentId || null)).length };
    setSettings(prev => ({ ...prev, orgChart: [...(prev.orgChart || []), node] }));
    setForm({ name: "", org: "", position: "", phone: "" });
  };

  const updateNode = () => {
    if (!editId || !form.name) return;
    setSettings(prev => ({ ...prev, orgChart: (prev.orgChart || []).map(n => n.id === editId ? { ...n, ...form } : n) }));
    setEditId(null); setForm({ name: "", org: "", position: "", phone: "" });
  };

  const deleteNode = (id) => {
    if (!confirm("삭제하시겠습니까? 하위 인원도 모두 삭제됩니다.")) return;
  ...
  const startEdit = (n) => { setEditId(n.id); setForm({ name: n.name, org: n.org || "", position: n.position || "", phone: n.phone || "" }); };

  const handleDrop = (targetId) => {
    if (!dragId || dragId === targetId) return;
    const isDescendant = (pid, cid) => { const ch = org.filter(n => n.parentId === pid); return ch.some(c => c.id === cid || isDescendant(c.id, cid)); };
    if (targetId && isDescendant(dragId, targetId)) return;
    setSettings(prev => ({ ...prev, orgChart: (prev.orgChart || []).map(n => n.id === dragId ? { ...n, parentId: targetId || null, order: prev.orgChart.filter(nn => nn.parentId === (targetId || null)).length } : n) }));
    setDragId(null);
  };
  ...
  const renderNode = (node, depth) => {
    ...
    return (
      <div key={node.id} style={{ marginLeft: depth * 24 }}>
        <div
          draggable onDragStart={(e) => { e.stopPropagation(); setDragId(node.id); }}
          onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); e.currentTarget.style.borderColor = "#2196F3"; }}
          onDragLeave={(e) => { e.currentTarget.style.borderColor = editId === node.id ? "rgba(33,150,243,0.3)" : "#222"; }}
          onDrop={(e) => { e.preventDefault(); e.stopPropagation(); e.currentTarget.style.borderColor = "#222"; handleDrop(node.id); }}
          style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", background: editId === node.id ? "rgba(33,150,243,0.08)" : dragId === node.id ? "rgba(255,152,0,0.08)" : "rgba(255,255,255,0.02)", borderRadius: 10, border: editId === node.id ? "1px solid rgba(33,150,243,0.3)" : "1px solid #222", marginBottom: 4, cursor: "grab", transition: "all .15s", flexWrap: "wrap" }}
        >
          {children.length > 0 ? <button onClick={() => setCollapsed(p => ({ ...p, [node.id]: !p[node.id] }))} style={{ background: "none", border: "none", color: "#556", fontSize: 12, cursor: "pointer", padding: 0, width: 18 }}>{isCol ? "▶" : "▼"}</button> : <span style={{ width: 18 }} />}
          {depth > 0 && <span style={{ color: "#333", fontSize: 10 }}>└</span>}
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              <span style={{ color: "#ccd6f6", fontSize: 14, fontWeight: 800 }}>{node.name}</span>
              {node.position && <span style={{ padding: "2px 8px", borderRadius: 4, background: "rgba(33,150,243,0.12)", color: "#2196F3", fontSize: 10, fontWeight: 700 }}>{node.position}</span>}
            </div>
            {node.org && <div style={{ color: "#556", fontSize: 11, marginTop: 2 }}>{node.org}</div>}
          </div>
          {node.phone && <a href={`tel:${node.phone.replace(/-/g, "")}`} onClick={(e) => e.stopPropagation()} style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 12px", borderRadius: 8, background: "rgba(76,175,80,0.1)", border: "1px solid rgba(76,175,80,0.2)", color: "#4CAF50", fontSize: 12, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" }}>📞 {node.phone}</a>}
          <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
            <button onClick={(e) => { e.stopPropagation(); startEdit(node); }} style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid #333", background: "transparent", color: "#8892b0", fontSize: 10, cursor: "pointer" }}>✏️</button>
            <button onClick={(e) => { e.stopPropagation(); deleteNode(node.id); }} style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid #a33", background: "transparent", color: "#F44336", fontSize: 10, cursor: "pointer" }}>🗑</button>
          </div>
        </div>
  ...
  return (<div>
    <Card>
      ...
      <p style={{ color: "#556", fontSize: 11, margin: "0 0 14px" }}>드래그하여 순서/계층 변경 | 📞 번호 터치 시 즉시 통화</p>
      <div style={{ padding: 14, borderRadius: 12, background: "rgba(255,255,255,0.02)", border: "1px solid #222", marginBottom: 16 }}>
        <div style={{ color: "#8892b0", fontSize: 12, fontWeight: 700, marginBottom: 10 }}>{editId ? "✏️ 수정" : "➕ 인원 추가"}</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
          <div><Label>이름 *</Label><Input id="org-name" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="홍길동" /></div>
          <div><Label>직책</Label><Input value={form.position} onChange={e => setForm(p => ({ ...p, position: e.target.value }))} placeholder="총괄책임자" /></div>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 10 }}>
          <div><Label>소속</Label><Input value={form.org} onChange={e => setForm(p => ({ ...p, org: e.target.value }))} placeholder="안전관리과" /></div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {editId ? (<>
            ...
            <button onClick={() => { setEditId(null); setForm({ name: "", org: "", position: "", phone: "" }); }} style={{ padding: "10px 16px", borderRadius: 8, border: "1px solid #333", background: "transparent", color: "#8892b0", cursor: "pointer" }}>취소</button>
          </>) : (<>
            <button onClick={() => addNode(null)} style={{ flex: 1, padding: "10px", borderRadius: 8, border: "none", background: "#4CAF50", color: "#fff", fontWeight: 700, cursor: "pointer" }}>최상위 추가</button>
            {org.length > 0 && <select onChange={e => { if (e.target.value) addNode(e.target.value); e.target.value = ""; }} style={{ padding: "10px", borderRadius: 8, border: "1px solid #333", background: "#111", color: "#fff", fontSize: 12 }}>
              <option value="">하위로 추가...</option>
              {org.map(n => <option key={n.id} value={n.id}>↳ {n.name} ({n.position || n.org || ""})</option>)}
            </select>}
          </>)}
        </div>
      </div>
      <div onDragOver={(e) => { e.preventDefault(); e.currentTarget.style.borderColor = "#FF9800"; }} onDragLeave={(e) => { e.currentTarget.style.borderColor = "#222"; }} onDrop={(e) => { e.preventDefault(); e.currentTarget.style.borderColor = "#222"; handleDrop(null); }} style={{ minHeight: 60, padding: 8, borderRadius: 10, border: "1px dashed #222", transition: "all .2s" }}>
        {roots.length === 0 && <p style={{ color: "#445", fontSize: 12, textAlign: "center", padding: 20 }}>조직도가 비어있습니다. 위에서 인원을 추가하세요.</p>}
    ...
    {org.length > 0 && <Card>
      ...
      <div style={{ display: "grid", gap: 4 }}>
        {org.map(n => (
          <div key={n.id} style={{ display: "flex", alignItems: "center", padding: "8px 12px", background: "rgba(255,255,255,0.02)", borderRadius: 8, gap: 10, flexWrap: "wrap" }}>
            <span style={{ color: "#ccd6f6", fontSize: 13, fontWeight: 700, minWidth: 60 }}>{n.name}</span>
            {n.position && <span style={{ color: "#2196F3", fontSize: 10, fontWeight: 600, minWidth: 60 }}>{n.position}</span>}
            {n.org && <span style={{ color: "#556", fontSize: 10, flex: 1 }}>{n.org}</span>}
            {n.phone && <a href={`tel:${n.phone.replace(/-/g, "")}`} style={{ display: "flex", alignItems: "center", gap: 4, padding: "6px 14px", borderRadius: 20, background: "rgba(76,175,80,0.1)", border: "1px solid rgba(76,175,80,0.2)", color: "#4CAF50", fontSize: 13, fontWeight: 700, textDecoration: "none" }}>📞 {n.phone}</a>}
          </div>
        ))}
      </div>
    </Card>}
