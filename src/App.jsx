...
const DEFAULT_CATEGORIES = [
  ...
  { id: "temp", name: "기온", unit: "°C", source: "api", icon: "🌡️", apiInterval: 10,
    ...
    isTempDual: true,
    currentValue: 20, actionItems: ["그늘막/방한용품 설치", "음료수/핫팩 배부", "의료진 대기 강화", "행사 중단 검토"],
...
function Dashboard({ categories, settings, onCardClick, onRefresh, alerts, onAction, onActionReport, onDeleteAlert, onDeleteNotice, userRole }) {
  ...
  return (<div style={{ minHeight: "100vh", background: "linear-gradient(145deg,#0a0a1a 0%,#0d1b2a 50%,#0a0a1a 100%)", padding: "24px 20px" }}>
    ...
    {/* 🅿️ 주차장 현황 */}
    {(settings.parkingLots || []).length > 0 && settings.dashboardVisible?.parking !== false && <>
      <div style={{ maxWidth: 1100, margin: "16px auto 8px" }}>
    ...
    {/* 🚌 셔틀버스 현황 */}
    {(settings.shuttleBuses || []).length > 0 && <>
      <div style={{ maxWidth: 1100, margin: "16px auto 8px" }}>
...
function CounterPage({ categories, setCategories, settings, setSettings, session }) {
  ...
  const cumTotal = settings.cumulativeVisitors || 0;
  const curTotal = crowd?.currentValue || 0;
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      const ms = Date.now();
      if (ms - lastHourlyRef.current < 300000) return;
      lastHourlyRef.current = ms;
      const entry = { time: fmtHM(new Date()), date: new Date().toLocaleDateString("ko-KR"), current: crowd?.currentValue || 0, cumulative: settings.cumulativeVisitors || 0, zones: zones.filter(z => z.name).map(z => ({ name: z.name, current: z.count || 0, cumulative: z.cumulative || 0 })) };
  ...
  const adjustTotal = (d) => {
    ...
    const newCur = Math.max(0, curTotal + d);
    const newCum = d > 0 ? cumTotal + d : cumTotal;
    setLog(p => [{ delta: d, time: ts, total: newCur, cum: newCum, zone: zoneName }, ...p].slice(0, 50));
    ...
    setSettings(prev => ({ ...prev, zones: newZones, cumulativeVisitors: newCum }));
    if (window.crowdDB) window.crowdDB.set(newCur, newZones, "counter").catch(() => {});
  };

  // 일일 마감 저장
  const saveDailyRecord = () => {
    const today = new Date().toLocaleDateString("ko-KR");
    const record = { date: today, cumulative: cumTotal, peakCurrent: curTotal, currentAtClose: curTotal, categories: categories.map(c => ({ name: c.name, icon: c.icon, value: c.currentValue, unit: c.unit })), zones: zones.filter(z => z.name).map(z => ({ name: z.name, cumulative: z.cumulative || 0, peak: z.count || 0 })) };
    setSettings(prev => ({ ...prev, dailyRecords: [...(prev.dailyRecords || []).filter(r => r.date !== today), record] }));
  ...
  const exportExcel = (type) => {
    ...
    if (type === "hourly" || type === "all") {
      ...
      const zNames = zones.filter(z => z.name).map(z => z.name);
  ...
  const Stat = ({ label, value, color, sub }) => (
    <div style={{ textAlign: "center" }}>
      ...
      {sub && <div style={{ color: "#445", fontSize: 9 }}>{sub}</div>}
    </div>
  );
  ...
  return (<div style={{ minHeight: "100vh", background: "#0a0a1a", display: "flex", flexDirection: "column", alignItems: "center", padding: "24px 16px" }}>
    ...
    {/* ★ 구역 배정된 계수원: 내 구역 먼저 */}
    {showZoneFirst && (() => { const z = zones.find(zz => zz.id === myZone.id); return z ? (
    ...
    {hasZones && <div style={{ width: "100%", maxWidth: 400, marginBottom: 14 }}>
      ...
      {selZone && !showZoneFirst && (() => { const z = zones.find(zz => zz.id === selZone); return z ? (
      ...
      <div style={{ display: "grid", gap: 4 }}>
        {zones.filter(z => z.name).map(z => (
          <div key={z.id} style={{ display: "flex", alignItems: "center", padding: "8px 12px", background: selZone === z.id ? "rgba(76,175,80,0.06)" : "rgba(255,255,255,0.02)", borderRadius: 8, border: selZone === z.id ? "1px solid rgba(76,175,80,0.2)" : "1px solid transparent" }}>
            <span style={{ color: "#ccd6f6", fontSize: 12, flex: 1 }}>{z.name}{z.assignee && <span style={{ color: "#445", fontSize: 10, marginLeft: 4 }}>({z.assignee})</span>}</span>
            ...
            <span style={{ color: "#445", fontSize: 10, marginLeft: 4 }}>/</span>
    ...
    <div style={{ width: "100%", maxWidth: 400, marginBottom: 14 }}>
      ...
      {showExport && <div style={{ marginTop: 8, padding: 14, borderRadius: 12, background: "rgba(255,255,255,0.02)", border: "1px solid #222", display: "grid", gap: 8 }}>
        ...
        <button onClick={() => exportExcel("hourly")} style={{ padding: "10px", borderRadius: 8, border: "1px solid #333", background: "transparent", color: "#8892b0", fontSize: 12, cursor: "pointer" }}>📥 시간별 현황 엑셀 다운로드</button>
        <button onClick={() => exportExcel("daily")} style={{ padding: "10px", borderRadius: 8, border: "1px solid #333", background: "transparent", color: "#8892b0", fontSize: 12, cursor: "pointer" }}>📥 일자별 현황 엑셀 다운로드</button>
        <button onClick={() => exportExcel("all")} style={{ padding: "10px", borderRadius: 8, border: "1px solid #2196F3", background: "rgba(33,150,243,0.08)", color: "#2196F3", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>📥 전체 데이터 엑셀 다운로드</button>
        <div style={{ color: "#445", fontSize: 10, lineHeight: 1.5, marginTop: 4 }}>
          시간별: 5분 간격 자동 기록 ({(settings.hourlyLog || []).length}건)<br/>
          일자별: 📋 금일 데이터 저장 클릭 시 기록 ({(settings.dailyRecords || []).length}건)
        </div>
      </div>}
    </div>
...
function CMSPage({ categories, setCategories, settings, setSettings, alerts, setAlerts, smsLog, initialTab, initialCatId, extraTabs, onExtraTab, userRole, accounts, setAccounts, onDataReset }) {
  ...
  const baseTabs = [
    ...
    { id: "sms", label: "📱 SMS" }, { id: "zones", label: "🗺️ 구역" }, { id: "workers", label: "👷 근무자" }, { id: "parking", label: "🅿️ 주차장" }, { id: "shuttlecms", label: "🚌 셔틀" }, { id: "custom", label: "➕ 항목" },
...
function AuthenticatedApp({ session, accounts, setAccounts, onLogout, initialPage, setPage: setPageExt }) {
  ...
  const allNavs = [
    ...
    { id: "parking", icon: "🅿️", label: "주차관리" },
    { id: "shuttle", icon: "🚌", label: "셔틀버스" },
    { id: "inbox", icon: "💬", label: unreadCount > 0 ? `수신함(${unreadCount})` : "수신함" },
    { id: "message", icon: "📢", label: "발송" },
