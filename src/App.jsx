...
function Dashboard({ categories: rawCategories, settings, onCardClick, onRefresh, alerts, onAction, onActionReport, onDeleteAlert, onDeleteNotice, userRole }) {
  ...
  // ★ crowd_realtime에서 직접 읽어서 categories의 crowd값 덮어쓰기
  const [crowdLive, setCrowdLive] = useState(() => {
    try { const c = JSON.parse(localStorage.getItem("_crowd") || "{}"); return c.total || 0; } catch { return 0; }
  });
  useEffect(() => {
    const load = async () => { try { const d = await window.crowdDB.get(); if (d) setCrowdLive(d.total || 0); } catch {} };
    load();
    const poll = setInterval(load, 5000);
    const handler = (e) => { if (e.detail?.total !== undefined) setCrowdLive(e.detail.total); };
