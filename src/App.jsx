...
function CounterPage({ categories, setCategories, settings, setSettings, session }) {
  ...
  // ★ 빠른 로컬 상태
  const catValue = crowd?.currentValue || 0;
  const [curTotal, setCurTotal] = useState(catValue);
  const [cumTotal, setCumTotal] = useState(0);
  const [zoneData, setZoneData] = useState(zones.map(z => ({ ...z })));
  const curRef = useRef(catValue);
  const cumRef = useRef(0);
  const zoneRef = useRef(zoneData);
  useEffect(() => { curRef.current = curTotal; }, [curTotal]);
  useEffect(() => { cumRef.current = cumTotal; }, [cumTotal]);
  useEffect(() => { zoneRef.current = zoneData; }, [zoneData]);

  // 초기 로드: categories와 crowdDB 중 큰 값 사용 + 양쪽 동기화
  useEffect(() => {
    const catVal = crowd?.currentValue || 0;
    if (window.crowdDB) {
      window.crowdDB.get().then(data => {
        // 체류: 둘 중 큰 값 사용 (stale 데이터 방지)
        const dbTotal = data?.total || 0;
        const best = Math.max(catVal, dbTotal);
        const bestCum = data?.cumulative || 0;
        setCurTotal(best); curRef.current = best;
        setCumTotal(bestCum); cumRef.current = bestCum;
        // 구역 데이터 복원
        if (data?.zones?.length) {
          const updated = zones.map(z => { const d = data.zones.find(dz => dz.id === z.id); return d ? { ...z, count: d.count || 0, cumulative: d.cumulative || 0 } : z; });
          setZoneData(updated); zoneRef.current = updated;
        }
        // categories 쪽도 동기화
        if (best !== catVal) {
          setCategories(p => p.map(c => c.id === "crowd" ? { ...c, currentValue: best } : c));
        }
        // crowdDB 쪽도 동기화
        if (best !== dbTotal) {
          const saveZones = (data?.zones || zones).map(z => ({ id: z.id, name: z.name, count: z.count || 0, cumulative: z.cumulative || 0 }));
          window.crowdDB.set(best, bestCum, saveZones, "sync");
        }
      });
    }
  }, []);
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.detail) {
        setCurTotal(e.detail.total || 0); curRef.current = e.detail.total || 0;
        setCumTotal(e.detail.cumulative || 0); cumRef.current = e.detail.cumulative || 0;
        if (e.detail.zones?.length) {
          setZoneData(prev => { const next = prev.map(z => { const d = e.detail.zones.find(dz => dz.id === z.id); return d ? { ...z, count: d.count || 0, cumulative: d.cumulative || 0 } : z; }); zoneRef.current = next; return next; });
        }
        setCategories(p => p.map(c => c.id === "crowd" ? { ...c, currentValue: e.detail.total || 0, lastUpdated: new Date().toLocaleTimeString("ko-KR") } : c));
      }
    };
  ...
  useEffect(() => { setZoneData(prev => zones.map(z => { const p = prev.find(pz => pz.id === z.id); return { ...z, count: p?.count || z.count || 0, cumulative: p?.cumulative || z.cumulative || 0 }; })); }, [zones.length]);
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      ...
      const entry = { time: fmtHM(new Date()), date: new Date().toLocaleDateString("ko-KR"), current: curRef.current, cumulative: cumRef.current, zones: zoneRef.current.filter(z => z.name).map(z => ({ name: z.name, current: z.count || 0, cumulative: z.cumulative || 0 })) };
      setSettings(prev => ({ ...prev, hourlyLog: [...(prev.hourlyLog || []).slice(-288), entry] }));
    }, 60000);
    return () => clearInterval(iv);
  }, []);

  // ★ 핵심: ref에서 읽어서 연타해도 즉시 반영
  const adjustTotal = (d) => {
    const newCur = Math.max(0, curRef.current + d);
    const newCum = d > 0 ? cumRef.current + d : cumRef.current;
    curRef.current = newCur;
    cumRef.current = newCum;
    setCurTotal(newCur);
    setCumTotal(newCum);
    setCategories(p => p.map(c => c.id === "crowd" ? { ...c, currentValue: newCur, lastUpdated: new Date().toLocaleTimeString("ko-KR") } : c));

    let newZones = zoneRef.current;
    if (selZone) {
      newZones = newZones.map(z => z.id === selZone ? { ...z, count: Math.max(0, (z.count || 0) + d), cumulative: d > 0 ? (z.cumulative || 0) + d : (z.cumulative || 0) } : z);
      zoneRef.current = newZones;
      setZoneData(newZones);
    }
    ...
    // crowdDB 즉시 저장
    if (window.crowdDB) {
      const saveZones = newZones.map(z => ({ id: z.id, name: z.name, count: z.count || 0, cumulative: z.cumulative || 0, range: z.range, assignee: z.assignee }));
      window.crowdDB.set(newCur, newCum, saveZones, session?.id || "counter");
    }
  };
...
function AuthenticatedApp({ session, accounts, setAccounts, onLogout, initialPage, setPage: setPageExt }) {
  ...
  // ★ 인파관리 실시간 동기화 (전용 테이블 — settings 경유 안 함)
  useEffect(() => {
