...
function Dashboard({ categories: rawCategories, settings, onCardClick, onRefresh, alerts, onAction, onActionReport, onDeleteAlert, onDeleteNotice, userRole }) {
  ...
  return (<div style={{ minHeight: "100vh", background: "linear-gradient(145deg,#0a0a1a 0%,#0d1b2a 50%,#0a0a1a 100%)", padding: "24px 20px" }}>
    ...
    {/* 📋 조직도 / 비상연락망 바로가기 */}
    {(settings.orgChart || []).length > 0 && <div style={{ maxWidth: 1100, margin: "12px auto 0" }}>
      <div onClick={() => { if (typeof window.__setPage === "function") window.__setPage("orgchart"); }} style={{ padding: "12px 16px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid #222", cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
        <span style={{ fontSize: 20 }}>📋</span>
        <div style={{ flex: 1 }}>
          <div style={{ color: "#ccd6f6", fontSize: 13, fontWeight: 700 }}>조직도 / 비상연락망</div>
          <div style={{ color: "#556", fontSize: 11 }}>{(settings.orgChart || []).length}명 등록 | 📞 터치 시 즉시 통화</div>
        </div>
...
function OrgChartTab({ settings, setSettings }) {
    ...
    return (
      <div key={node.id} style={{ marginLeft: depth * 20 }}>
        <div draggable onDragStart={(e) => { e.stopPropagation(); setDragId(node.id); }}
          ...
          {children.length > 0 ? <button onClick={() => setCollapsed(p => ({ ...p, [node.id]: !p[node.id] }))} style={{ background: "none", border: "none", color: isOrg ? "#2196F3" : "#556", fontSize: 12, cursor: "pointer", padding: 0, width: 18 }}>{isCol ? "▶" : "▼"}</button> : <span style={{ width: 18 }} />}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              ...
              {node.position && <span style={{ padding: "2px 8px", borderRadius: 4, background: isOrg ? "rgba(33,150,243,0.12)" : "rgba(76,175,80,0.12)", color: isOrg ? "#2196F3" : "#4CAF50", fontSize: 10, fontWeight: 700 }}>{node.position}</span>}
            </div>
            {node.memo && <div style={{ color: "#556", fontSize: 10, marginTop: 2 }}>{node.memo}</div>}
          </div>
  ...
  return (<div>
    <Card>
      ...
      <p style={{ color: "#556", fontSize: 11, margin: "0 0 14px" }}>조직을 먼저 만들고, 인원을 해당 조직에 배치하세요. 드래그로 이동 가능.</p>
      ...
      <div style={{ padding: 14, borderRadius: 12, background: "rgba(255,255,255,0.02)", border: `1px solid ${editId ? "rgba(33,150,243,0.3)" : "#222"}`, marginBottom: 16 }}>
        <div style={{ color: "#8892b0", fontSize: 12, fontWeight: 700, marginBottom: 10 }}>{editId ? `✏️ ${addMode === "org" ? "조직" : "인원"} 수정` : addMode === "org" ? "🏢 조직 추가" : "👤 인원 추가"}</div>
      ...
      <div onDragOver={(e) => { e.preventDefault(); e.currentTarget.style.outline = "2px dashed #FF9800"; }} onDragLeave={(e) => { e.currentTarget.style.outline = "none"; }} onDrop={(e) => { e.preventDefault(); e.currentTarget.style.outline = "none"; handleDrop(null); }} style={{ minHeight: 60, padding: 8, borderRadius: 10, border: "1px dashed #222" }}>
        {roots.length === 0 && <p style={{ color: "#445", fontSize: 12, textAlign: "center", padding: 20 }}>🏢 조직을 먼저 추가한 후, 👤 인원을 배치하세요.</p>}
      ...
      <div style={{ display: "grid", gap: 4 }}>
        {org.filter(n => n.type === "person" && n.phone).map(n => {
          ...
          return (<div key={n.id} style={{ display: "flex", alignItems: "center", padding: "8px 12px", background: "rgba(255,255,255,0.02)", borderRadius: 8, gap: 10, flexWrap: "wrap" }}>
            ...
            {n.position && <span style={{ color: "#4CAF50", fontSize: 10, fontWeight: 600, minWidth: 50 }}>{n.position}</span>}
            {parentOrg && <span style={{ color: "#556", fontSize: 10, flex: 1 }}>🏢 {parentOrg.name}</span>}
...
// ─── OrgChart Page (standalone) ──────────────────────────────────
function OrgChartPage({ settings, setSettings, session }) {
  const org = settings.orgChart || [];
  const isEditor = session?.role === "admin" || session?.role === "manager";
  const [collapsed, setCollapsed] = useState({});
  const getChildren = (pid) => org.filter(n => n.parentId === pid).sort((a, b) => {
    if (a.type !== b.type) return a.type === "org" ? -1 : 1;
    return (a.order || 0) - (b.order || 0);
  ...
  const renderNode = (node, depth) => {
    ...
    return (
      <div key={node.id} style={{ marginLeft: depth * 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: isOrg ? "10px 14px" : "8px 12px", background: isOrg ? "rgba(33,150,243,0.06)" : "rgba(255,255,255,0.02)", borderRadius: 10, border: isOrg ? "1px solid rgba(33,150,243,0.15)" : "1px solid #222", marginBottom: 4, flexWrap: "wrap" }}>
          {children.length > 0 ? <button onClick={() => setCollapsed(p => ({ ...p, [node.id]: !p[node.id] }))} style={{ background: "none", border: "none", color: isOrg ? "#2196F3" : "#556", fontSize: 12, cursor: "pointer", padding: 0, width: 18 }}>{isCol ? "▶" : "▼"}</button> : <span style={{ width: 18 }} />}
          {depth > 0 && <span style={{ color: "#333", fontSize: 10 }}>└</span>}
          <span style={{ fontSize: 16 }}>{isOrg ? "🏢" : "👤"}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              <span style={{ color: isOrg ? "#2196F3" : "#ccd6f6", fontSize: isOrg ? 15 : 14, fontWeight: 800 }}>{node.name}</span>
              {node.position && <span style={{ padding: "2px 8px", borderRadius: 4, background: isOrg ? "rgba(33,150,243,0.12)" : "rgba(76,175,80,0.12)", color: isOrg ? "#2196F3" : "#4CAF50", fontSize: 10, fontWeight: 700 }}>{node.position}</span>}
            </div>
            {node.memo && <div style={{ color: "#556", fontSize: 10, marginTop: 2 }}>{node.memo}</div>}
          </div>
          {node.phone && <a href={`tel:${node.phone.replace(/-/g, "")}`} style={{ display: "flex", alignItems: "center", gap: 4, padding: "8px 16px", borderRadius: 10, background: "rgba(76,175,80,0.12)", border: "1px solid rgba(76,175,80,0.25)", color: "#4CAF50", fontSize: 14, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" }}>📞 {node.phone}</a>}
        </div>
        {!isCol && children.map(c => renderNode(c, depth + 1))}
      </div>
    );
  ...
  if (isEditor) return (<div style={{ minHeight: "100vh", background: "#0d1117", padding: "20px 16px" }}>
    <h2 style={{ color: "#fff", fontSize: 20, fontWeight: 800, textAlign: "center", margin: "0 0 14px" }}>📋 조직도 / 비상연락망</h2>
    <div style={{ maxWidth: 700, margin: "0 auto" }}><OrgChartTab settings={settings} setSettings={setSettings} /></div>
  </div>);

  return (<div style={{ minHeight: "100vh", background: "#0d1117", padding: "20px 16px" }}>
    <h2 style={{ color: "#fff", fontSize: 20, fontWeight: 800, textAlign: "center", margin: "0 0 4px" }}>📋 조직도 / 비상연락망</h2>
    <p style={{ color: "#8892b0", fontSize: 12, textAlign: "center", margin: "0 0 20px" }}>📞 번호를 터치하면 바로 전화합니다</p>
    <div style={{ maxWidth: 700, margin: "0 auto" }}>
      {org.length === 0 && <div style={{ textAlign: "center", padding: 40, color: "#556" }}>조직도가 등록되지 않았습니다.</div>}
      {roots.map(r => renderNode(r, 0))}
    </div>
  </div>);
}
...
const ROLES = {
  admin: { label: "관리자", color: "#F44336", pages: ["dashboard", "counter", "parking", "shuttle", "message", "inbox", "orgchart", "cms"], desc: "모든 기능 접근" },
  manager: { label: "운영자", color: "#FF9800", pages: ["dashboard", "counter", "parking", "shuttle", "message", "inbox", "orgchart", "cms"], desc: "설정 변경 가능 (계정관리 제외)" },
  counter: { label: "계수원", color: "#4CAF50", pages: ["counter", "dashboard", "inbox", "orgchart"], desc: "인파 계수 + 대시보드 조회" },
  parking: { label: "주차요원", color: "#9C27B0", pages: ["parking", "dashboard", "inbox", "orgchart"], desc: "주차장 관리 + 대시보드 조회" },
  shuttle: { label: "셔틀요원", color: "#00BCD4", pages: ["shuttle", "dashboard", "inbox", "orgchart"], desc: "셔틀버스 위치 관리" },
  viewer: { label: "뷰어", color: "#2196F3", pages: ["dashboard", "inbox", "orgchart"], desc: "대시보드 조회만 가능" },
};
...
function AppMain({ onError }) {
  ...
  window.__setPage = setPage;  // Dashboard 조직도 버튼용
...
function AuthenticatedApp({ session, accounts, setAccounts, onLogout, initialPage, setPage: setPageExt }) {
  ...
  return (<div style={{ fontFamily: "'Noto Sans KR',-apple-system,sans-serif" }}>
    ...
    <div style={{ paddingTop: 36, paddingBottom: 70 }}>
      ...
      {page === "orgchart" && <OrgChartPage settings={settings} setSettings={setSettings} session={session} />}
