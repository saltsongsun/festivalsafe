...
function Dashboard({ categories: rawCategories, settings, onCardClick, onRefresh, alerts, onAction, onActionReport, onDeleteAlert, onDeleteNotice, userRole }) {
  ...
  return (<div style={{ minHeight: "100vh", background: "linear-gradient(145deg,#0a0a1a 0%,#0d1b2a 50%,#0a0a1a 100%)", padding: "24px 20px" }}>
    ...
    {/* 📋 조직도 / 비상연락망 */}
    {(settings.orgChart || []).length > 0 && (() => {
      const orgData = settings.orgChart || [];
      const orgPersons = orgData.filter(n => n.type === "person" && n.phone);
      const getChildren = (pid) => orgData.filter(n => n.parentId === pid).sort((a, b) => { if (a.type !== b.type) return a.type === "org" ? -1 : 1; return (a.order || 0) - (b.order || 0); });
      const roots = orgData.filter(n => !n.parentId).sort((a, b) => { if (a.type !== b.type) return a.type === "org" ? -1 : 1; return (a.order || 0) - (b.order || 0); });
      const renderOrgNode = (node, depth) => {
        const children = getChildren(node.id);
        const isOrg = node.type === "org";
        return (<div key={node.id} style={{ marginLeft: depth * 18 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: isOrg ? "8px 12px" : "6px 10px", background: isOrg ? "rgba(33,150,243,0.05)" : "transparent", borderRadius: 8, marginBottom: 3, flexWrap: "wrap" }}>
            <span style={{ fontSize: 14 }}>{isOrg ? "🏢" : "👤"}</span>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                <span style={{ color: isOrg ? "#2196F3" : "#ccd6f6", fontSize: isOrg ? 13 : 12, fontWeight: 700 }}>{node.name}</span>
                {node.position && <span style={{ padding: "1px 6px", borderRadius: 4, background: isOrg ? "rgba(33,150,243,0.1)" : "rgba(76,175,80,0.1)", color: isOrg ? "#2196F3" : "#4CAF50", fontSize: 9, fontWeight: 700 }}>{node.position}</span>}
              </div>
            </div>
            {node.phone && <a href={`tel:${node.phone.replace(/-/g, "")}`} style={{ padding: "5px 10px", borderRadius: 8, background: "rgba(76,175,80,0.1)", border: "1px solid rgba(76,175,80,0.2)", color: "#4CAF50", fontSize: 11, fontWeight: 700, textDecoration: "none", whiteSpace: "nowrap" }}>📞 {node.phone}</a>}
          </div>
          {children.map(c => renderOrgNode(c, depth + 1))}
        </div>);
      };
      return (<div style={{ maxWidth: 1100, margin: "12px auto 0" }}>
        <div onClick={() => setShowOrgChart(!showOrgChart)} style={{ padding: "12px 16px", borderRadius: showOrgChart ? "12px 12px 0 0" : 12, background: "rgba(255,255,255,0.03)", border: "1px solid #222", cursor: "pointer", display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 20 }}>📋</span>
          <div style={{ flex: 1 }}>
            <div style={{ color: "#ccd6f6", fontSize: 13, fontWeight: 700 }}>조직도 / 비상연락망</div>
            <div style={{ color: "#556", fontSize: 11 }}>{orgData.filter(n => n.type === "org").length}개 조직 · {orgData.filter(n => n.type === "person").length}명</div>
          </div>
          <span style={{ color: "#556", fontSize: 12 }}>{showOrgChart ? "▲" : "▼"}</span>
        </div>
        {showOrgChart && <div style={{ padding: 14, background: "rgba(255,255,255,0.02)", border: "1px solid #222", borderTop: "none", borderRadius: "0 0 12px 12px" }}>
          {roots.map(r => renderOrgNode(r, 0))}
          {orgPersons.length > 0 && <>
            <div style={{ borderTop: "1px solid #222", margin: "12px 0 8px", paddingTop: 10 }}>
              <span style={{ color: "#8892b0", fontSize: 12, fontWeight: 700 }}>📞 비상연락망 (빠른 통화)</span>
            </div>
            <div style={{ display: "grid", gap: 3 }}>
              {orgPersons.map(n => {
                const parentOrg = orgData.find(o => o.id === n.parentId && o.type === "org");
                return (<div key={n.id} style={{ display: "flex", alignItems: "center", padding: "6px 10px", borderRadius: 6, gap: 8, flexWrap: "wrap" }}>
                  <span style={{ color: "#ccd6f6", fontSize: 12, fontWeight: 700, minWidth: 50 }}>{n.name}</span>
                  {n.position && <span style={{ color: "#4CAF50", fontSize: 10, minWidth: 50 }}>{n.position}</span>}
                  {parentOrg && <span style={{ color: "#556", fontSize: 10, flex: 1 }}>🏢 {parentOrg.name}</span>}
                  <a href={`tel:${n.phone.replace(/-/g, "")}`} style={{ padding: "5px 12px", borderRadius: 20, background: "rgba(76,175,80,0.1)", border: "1px solid rgba(76,175,80,0.2)", color: "#4CAF50", fontSize: 12, fontWeight: 700, textDecoration: "none" }}>📞 {n.phone}</a>
                </div>);
              })}
            </div>
          </>}
