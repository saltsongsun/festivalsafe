...
window.crowdDB = supabase ? {
  async get() {
    // ★ localStorage가 항상 최신 (set에서 동기 저장)
    ...
    try {
      ...
      const result = data || { total: 0, cumulative: 0, zones: [] }
      localStorage.setItem('_crowd', JSON.stringify(result))
      return result
  ...
  async get() { return JSON.parse(localStorage.getItem('_crowd') || '{"total":0,"cumulative":0,"zones":[]}') },
  async set(total, cumulative, zones, updatedBy) {
...
// ─── Realtime 구독 ───────────────────────────────────────────────
if (supabase) {
  // ★ app_state: 이벤트만 전달 (localStorage 덮어쓰기 안 함 → 타이핑 깨짐 방지)
  ...
  // crowd_realtime: 계수원 입력은 즉시 반영 필요
